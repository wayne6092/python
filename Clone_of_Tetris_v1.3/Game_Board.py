...
shape_8_image = pygame.image.load('shape_8_image.png').convert()#new shape
shape_8_image.set_colorkey((255,0,255))
...
class GameBoard:
    def __init__(self):
        ...
        self.next_gp = None
        self.queue_image = pygame.Surface((162, 162))#second window
    ...
    def reset(self, time):
        ...
        self.generate_gp(time)#gamepiece is here too

    def generate_gp(self, time):# Generates Game piece
        self.key_down_flag = False
        self.key_left_flag = False
        self.key_right_flag = False
        shape_id = random.randint(1,8)#Add one to range when creating NEW blocks
        rand_rotate = random.randint(0,5)
        ...
        self.queue_image.blit(queue_bg, (0,0))#Blits image of the blocks
        for gp in self.next_gp.pieces:
    ...
    def shift_row(self, row_number):
        for row in range(row_number, 0, -1):#If below the last row then self.cells[row][col]
            for col in range(self.cols):
                self.cells[row][col].active = self.cells[row-1][col].active #changes the row to below last
                self.cells[row][col].image = self.cells[row-1][col].image #changes image of row below last
                print('awesome')
    ...
    def draw(self, surface):
        ...
        y = self.cell_height
        surface.blit(self.queue_image, (x,y))#Draws the queue to the screen
        ...
        x += ((self.queue_image.get_width()/2) - (font_w/2))#Sets queue image
        y -= font_h
